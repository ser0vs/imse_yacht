...
def populate_database(cursor):
    ...
    for _ in range(30):
        ...
        cursor.execute("INSERT INTO Yacht (yachtID, model, length, orderID) VALUES (%s, %s, %s, %s)",
    ...
    used_pairs = set()  # Track existing (orderID, employeeID) pairs
    for _ in range(20):
        while True:
            order_id = fake.random_element(orders)
            employee_id = fake.random_element(builders)
            if (order_id, employee_id) not in used_pairs:  # Ensure uniqueness
                used_pairs.add((order_id, employee_id))
                cursor.execute("INSERT INTO CustomerOrderBuilder (orderID, employeeID) VALUES (%s, %s)", 
